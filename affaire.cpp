...
{
    QSqlQuery query;
          query.prepare("INSERT INTO AFFAIRE (ID,TYPE,DATE_DE_DEPOT) "
...
bool Affaires::modifier(QString id)
{
    ...
    query.prepare("UPDATE AFFAIRE SET ID='"+id+"', TYPE='"+type+"', DATE_DE_DEPOT='"+date+"' where ID='"+id+"'");
           /* query.bindValue(0, id);
...
QSqlQueryModel* Affaires::tri(){
           /* QSqlQuery qry;
             qry.prepare("SELECT * FROM AFFAIRE where ID=:ID ORDER BY TYPE");
             qry.bindValue(":ID", id);
             qry.exec();
             QSqlQueryModel *model= new QSqlQueryModel;
             model->setQuery(qry);
